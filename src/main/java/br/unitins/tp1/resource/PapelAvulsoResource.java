...
import br.unitins.tp1.service.PapelAvulsoService;
...
public class PapelAvulsoResource {


    @Inject //injeção de dependência
    PapelAvulsoService service;

    @GET
    public List<PapelAvulso> buscarTodos() {
        return service.findAll();
    }

    @GET
    @Path ("/find/{textura}") //Vai adicionar uma segunda camada de recurso: site.com/papeis/find/{textura}
    public List <PapelAvulso> buscarPorTextura (Textura textura) {
        return service.findByTextura(textura);
    }

    @RolesAllowed("Administrador")
    @POST
    public PapelAvulsoDTOResponse incluir (PapelAvulsoDTO dto){
    ...
    @Path("/{id}")
    @Transactional
    public void editar(Long id, PapelAvulsoDTO dto){
